...
$$
**And by partial derivatives, $\vec{\mu}^{(t+1)}$ is $\vec{x} \cdot \Omega^{(t)}$ divided element-wise by the column sums of $\Omega^{(t)}$, and $\vec{\sigma^2}^{(t+1)}$ is the diagonal of $([1,...,1]^T \cdot \vec{x} - \vec{\mu}^{(t+1)} \cdot [1,...,1])^2 \cdot \Omega^{(t)}$ divided element-wise by the column sums of $\Omega^{(t)}$.**
...
**Both $\theta_3$ and $\theta_4$ yield a satisfactory estimate for the Gaussian mixture model that $X$ is sampled from. Moreover, they can be directly computed from $X$ and the value of $K$ we set, and thus automate the initialization.**
