...
\[
\epsilon_{IS} \approx 1.75 \times 10^{-3}
\]
...
I_true <- integrate(gg, lower=-Inf, upper=Inf)
truth <- I_true$value
print(paste("The true I is:", truth, 
            "with absolute error <", I_true$abs.error))
eps <- mean(g(rnorm(10000)))-truth
cat("Naive Simulation: approximation error =",eps,"\n")
cat("Importance Sampling: approximation error =",
    I_IS-truth)
```
...
```{r approx error Anti}
cat("Antithetic Variables: approximation error =",
    I_anti - truth)
```

\[
|\epsilon_{IS}| < |\epsilon_{anti}| \approx 3.56 \times 10^{-3}
\]
**The approximation error is larger by Antithetic Variables than by Importance Sampling.**
...
```{r compare errors}
COMPARE <- data.frame("Approximation Error" = c(eps,
                                                I_IS-truth,
                                              I_anti-truth,
                                            I_comb-truth))
...
```
\[
\epsilon_{comb} \approx - 3.90 \times 10^{-4}
\]
